...
Interleave tt::grid::interleaveFromString(const std::string& str) {
	std::string lower;
...
size_t tt::grid::detail::minValue(std::unordered_map<size_t, double>& m) {
...
void tt::grid::detail::polyWriteToDB(PolygonContext* pc) {
	...
	while(!pc->monitor->canceled() && (pc->writeRunning || !pc->geoms.empty())) {
		...
		for(PolygonValue v : pc->fieldValues) {
			...
			case OFTWideString:
				...
				break;
			default:
				_runerr("Unknown field type: " << v.type);
			}
		}
		...
		if(OGRERR_NONE != err)
			_runerr("Failed to add geometry.");
	}
}

void tt::grid::detail::printGEOSGeom(GEOSGeometry* geom, GEOSContextHandle_t gctx) {
...
GEOSGeometry* tt::grid::detail::polyMakeGeom(GEOSContextHandle_t gctx, double x0, double y0, double x1, double y1, double eps, int dims) {
...
void tt::grid::detail::polyMakeDataset(const std::string& filename, const std::string& driver,
...
int tt::grid::detail::getTypeSize(DataType type) {
	switch(type) {
	...
	default:
		_runerr("No size for type: " << (int) type);
	}
}

GDALDataType tt::grid::detail::dataType2GDT(DataType type) {
	switch(type) {
...
DataType tt::grid::detail::gdt2DataType(GDALDataType type) {
	switch(type) {
...
void tt::grid::detail::fixWriteBounds(int& cols, int& rows, int& srcCol, int& srcRow,
...
bool tt::grid::detail::fixCoords(int& srcCol, int& srcRow, int& dstCol, int& dstRow,
	...
	if(srcCol >= srcCols || srcRow >= srcRows || srcCol + cols < 0 || srcRow + rows < 0) {
		_warn("Col/row out of range." << srcCol << ", " << srcRow << ", " << srcCols << ", " << srcRows << ", " << cols << ", " << rows);
		return false;
	}
	if(srcCol < 0) {
		cols += srcCol;
	...
	if(cols <= 0 || rows <= 0) {
		_warn("Zero copy area.")
		return false;
	}
	return true;
}

int tt::grid::detail::gdalProgress(double dfComplete, const char *pszMessage, void *pProgressArg) {
