...
public:
	...
	std::ofstream out;
	bool inited = false;
	double bounds[6] = { G_DBL_MAX_POS, G_DBL_MAX_POS, G_DBL_MAX_NEG,
	...
	bool near(Seg *seg) {
		return ((start - seg->end) < 1.0 && start > seg->end)
				|| ((seg->start - end) < 1.0 && seg->start > end);
	}

	bool insert(Seg *seg) {
		g_trace(
				"Insert: " << seg->id << ": " << seg->start << " > "
						<< seg->end);
		if (seg->id == id) {
			g_trace("> same seg.");
			return false;
...
void mapClasses(std::vector<std::string> &files, std::string &outfile,
		std::map<int, int> &mappings) {

	if (mappings.size() == 0)
		g_argerr("At least one mapping is required.");
	g_trace("Mappings:");
	for (std::map<int, int>::iterator it = mappings.begin();
			it != mappings.end(); ++it)
...
void recoverFlightlines(std::vector<std::string> &files, std::string &outfile,
		double timeGap, bool exportLas) {
		...
		while (r.ReadNextPoint()) {
			...
			if (startTime == 0) {
				...
				if (gap < 0.0 || gap > timeGap) { // One what?
					...
					g_trace(
							"Time gap. Flightline: " << gap << "; " << startTime
									<< " > " << endTime);
					startTime = time;
				}
				endTime = time;
...
void recoverEdgesWEdgeMarker(std::vector<std::string> &files,
		std::string &outfile) {
...
void usage() {
	std::cerr << "Usage: lasreclass [options] srcfiles*\n"
			...
			<< " -fg             The time gap required to signify a break in flight lines. Defaul "
			<< TIME_GAP << "\n";
}
...
int main(int argc, char ** argv) {
	...
	try {
		...
		if (mapping) {
			...
			geotools::las::recoverFlightlines(files, outfile, flTimeGap,
					flExport);
