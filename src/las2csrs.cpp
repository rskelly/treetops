...
static double _binterp(float *grid, double c, double r, int c0, int r0, int c1,
		int r1, int width) {
	double x1 = (c1 - c) / (c1 - c0) * grid[r0 * width + c0]
			+ (c - c0) / (c1 - c0) * grid[r0 * width + c1];
	double x2 = (c1 - c) / (c1 - c0) * grid[r1 * width + c0]
			+ (c - c0) / (c1 - c0) * grid[r1 * width + c1];
 ...
 */
static void _shift2latlon(Grid<double> &gdx, Grid<double> &gdy,
		Grid<double> &glat, Grid<double> &gh, double a, double e2, int count,
		Grid<double> &gdlat, Grid<double> &gdlon) {
...
public:
	...
	void load() {
		...
		if (!ds) {
			char path[PATH_MAX];
			sprintf(path, "%s%s", std::getenv("LAS2CSRS_DATA"),
					"/NAD83v6VG.tif");
			ds = (GDALDataset *) GDALOpen(path, GA_ReadOnly);
		}
		if (!ds)
			g_runerr(
					"Failed to load shift grid. Set the LAS2CSRS_DATA variable to point to the itrf database and grid shift file.");
		...
		CPLErr xe = xband->RasterIO(GF_Read, 0, 0, width, height, xg.grid(),
				width, height, GDT_Float32, 0, 0);
		CPLErr ye = yband->RasterIO(GF_Read, 0, 0, width, height, yg.grid(),
				width, height, GDT_Float32, 0, 0);
		CPLErr ze = zband->RasterIO(GF_Read, 0, 0, width, height, zg.grid(),
				width, height, GDT_Float32, 0, 0);
...
public:
	...
	Params() :
			ffrom(""), efrom(0), eto(0), fromSRS(nullptr), toSRS(nullptr), tx(
					0), ty(0), tz(0), dtx(0), dty(0), dtz(0), rx(0), ry(0), rz(
					0), drx(0), dry(0), drz(0), epoch(0), d(0), dd(0) {
	}
...
private:
	 ...
	 */
	void epochTransform(Params &params, Grid<double> &gx, Grid<double> &gy,
			Grid<double> &gz, int count, double dt) {
		...
		g_trace(
				"  > Txt: " << Txt << "; Tyt: " << Tyt << "; Tzt: " << Tzt
						<< "\n" << "  > Rxt: " << Rxt << "; Ryt: " << Ryt
						<< "; Rzt: " << Rzt << "\n" << "  > DSt: " << DSt
						<< "\n" << "  > drx: " << params.drx << "; dry: "
						<< params.dry << "; drz: " << params.drz << "\n");
	...
	void loadHelmert(Params &p) {
		...
		std::string ffrom;
		std::string fto;
		float epoch, tx, ty, tz, rx, ry, rz, d, dtx, dty, dtz, drx, dry, drz,
				dd;
		bool found = false;
		std::ifstream f;
		...
		if (!f.is_open())
			g_runerr(
					"Failed to open itrf database. If necessary, set LAS2CSRS_DATA environment variable.");
		std::string line;
		...
		if (!found)
			g_argerr(
					"Failed to find a transformation matching the parameters.");

		g_trace(
				" -- Params: dtx: " << p.dtx << "; dty: " << p.dty << "; dtz: "
						<< p.dtz);
		g_trace(
				"            drx: " << p.drx << "; dry: " << p.dry << "; drz: "
						<< p.drz);
...
public:
	 ...
	 */
	Transformer(const std::string &ffrom, float efrom, float eto,
			const std::string &fromSRS, const std::string &toSRS) {
		...
		if (eto < 1980)
			g_argerr(
					"A destination epoch is required (should be 1980 or higher.)");
		if (toSRS.empty())
			g_argerr(
					"A destination SRS is required. Include geoid information if orthometric output is required.");
		if (fromSRS.empty())
			g_warn("Attempting to load source SRS information from LAS files.");

		g_trace(
				"Transformer:\n -- src ref frame: " << ffrom << "; src epoch: "
						<< efrom << "; dst epoch: " << eto << " -- from srs: "
						<< fromSRS << "; to srs: ");
	 ...
	 */
	void transformPoints(Grid<double> &x, Grid<double> &y, Grid<double> &z,
			int count, double bounds[6]) {
		...
		g_trace(
				"transformPoints" << " -- Original: " << x.grid()[0] << ", "
						<< y.grid()[0] << ", " << z.grid()[0]);

		// 1) Project to ECEF in the original reference frame.
		pj_transform(projFrom, projECEF, count, 1, x.grid(), y.grid(),
				z.grid());
		g_trace(
				" -- ECEF (Original): " << x.grid()[0] << ", " << y.grid()[0]
						<< ", " << z.grid()[0]);

		// 2) Transform to NAD83 @ 1997.
		epochTransform(params, x, y, z, count, params.efrom - params.epoch);
		g_trace(
				" -- ECEF (CSRS): " << x.grid()[0] << ", " << y.grid()[0]
						<< ", " << z.grid()[0]);
		...
		if (params.efrom != params.eto) {
			...
			pj_transform(projECEF, projGeog, count, 1, x0.grid(), y0.grid(),
					z0.grid());

			g_trace(
					" -- Lat Lon (CSRS): " << g_deg(x0.grid()[0]) << ", "
							<< g_deg(y0.grid()[0]) << ", " << z0.grid()[0]);
			...
			g_trace(
					" -- Lat lon shifted: " << g_deg(x0.grid()[0]) << ", "
							<< g_deg(y0.grid()[0]) << ", " << z0.grid()[0]);

			// Transform latlon to target proj
			pj_transform(projGeog, projTo, count, 1, x0.grid(), y0.grid(),
					z0.grid());
			...
			pj_transform(projECEF, projTo, count, 1, x.grid(), y.grid(),
					z.grid());

		}

		g_trace(
				" -- Final: " << x.grid()[0] << ", " << y.grid()[0] << ", "
						<< z.grid()[0]);
	 ...
	 */
	int transformLas(std::vector<std::string> &srcfiles, std::string &dstdir,
			bool overwrite) {
		...
		for (std::string filename : srcfiles) {
			...
			if (!params.fromSRS) {
				...
				if (!params.fromSRS)
					g_argerr(
							"SRS not provided and not found on first LAS file.");
			}
			...
			std::ofstream out(dstfile.c_str(),
