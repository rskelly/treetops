 ...
 */
bool feather(Grid<float> &srcGrid, Grid<float> &dstGrid, float distance,
 ...
 */
void blend(Grid<float> &imGrid, Grid<float> &bgGrid, Grid<float> &alpha,
...
public:
	...
	void writeOutput(MemRaster<float> &buf, Raster<float> &output) const {
		if (!(oCol >= output.cols() || oRow >= output.rows()))
			output.writeBlock(oCol, oRow, buf, buffer, buffer, tileSize,
					tileSize);
	}
...
void Mosaic::mosaic(const std::vector<std::string> &files,
	...
	for (unsigned int i = 1; i < files.size(); ++i) {
		...
		int iStartCol = input.toCol(
				input.positiveX() ? bounds.minx() : bounds.maxx());
		int iStartRow = input.toRow(
				input.positiveY() ? bounds.miny() : bounds.maxy());
		int iEndCol = input.toCol(
				input.positiveX() ? bounds.maxx() : bounds.minx());
		int iEndRow = input.toRow(
				input.positiveY() ? bounds.maxy() : bounds.miny());
		int oStartCol = base.toCol(
				output.positiveX() ? bounds.minx() : bounds.maxx());
		int oStartRow = base.toRow(
...
#pragma omp parallel shared(tileStatus)
		{
			...
			MemRaster<float> inGrid(tileSize + buffer * 2,
					tileSize + buffer * 2);
			MemRaster<float> outGrid(tileSize + buffer * 2,
					tileSize + buffer * 2);
...
#pragma omp atomic
				...
				if (m_callbacks)
					m_callbacks->stepCallback(
							(tileStatus - 0.5) / tiles.size());

				std::unique_ptr<Tile> tile = std::move(tiles[t]);

				g_debug(
						" -- tile " << t << ", " << tile->id << "; thread: "
								<< omp_get_thread_num());
