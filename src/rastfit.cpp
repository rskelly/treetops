...
public:
	...
	samplesort(Raster<float> &r) :
			r(r) {
	}

	bool operator()(std::unique_ptr<InterpPoint> &a,
			std::unique_ptr<InterpPoint> &b) const {
		return (a.get(), b.get());
	}
...
void generateMaskSamples(std::list<InterpPoint> &samples, Raster<char> &mask,
	...
	for (int r = 0; r < mask.rows(); ++r) {
		for (int c = 0; c < mask.cols(); ++c) {
			if (mask.get(c, r) == 1.0 && adj.isValid(mask.toX(c), mask.toY(r))
					&& base.isValid(mask.toX(c), mask.toY(r))) {
				InterpPoint p(mask.toX(c), mask.toY(r));
				p.z = adj.get(mask.toX(c), mask.toY(r))
						- base.get(mask.toX(c), mask.toY(r));
				pts.push_back(p);
			}
		}
	}
	...
	samples.assign(pts.begin(),
			pts.begin() + (numSamples > pts.size() ? pts.size() : numSamples));
}

/**
 * Generates a list of random samples within the bounds of a raster.
 */
void generateRandomSamples(std::list<InterpPoint> &samples, Raster<float> &adj,
		Raster<float> &base, double *bounds, unsigned int numSamples) {
	while (numSamples-- > 0) {
...
void printSamples(std::list<InterpPoint> &samples) {
	...
	for (auto it = samples.begin(); it != samples.end(); ++it) {
		d += g_sq(it->z);
		r += g_sq(it->resid);
		std::cout << std::setprecision(9) << it->x << "," << it->y << ","
				<< it->z << "," << it->resid << std::endl;
	}
	std::cout << "Var before: " << (d / samples.size()) << ", after: "
			<< (r / samples.size()) << std::endl;
}
...
void adjust(std::string &basefile, std::string &adjfile, std::string &maskfile,
		std::string &outfile, interp::Interpolator &inter,
	...
	std::string proj;
	adj.projection(proj);
	Raster<float> out(outfile, adj.minx(), adj.miny(), adj.maxx(), adj.maxy(),
			resolution, adj.nodata(), proj);
...
void usage() {
	std::cerr << "Usage: rastfit [options]" << std::endl
			<< "This program will produce an adjustment raster to adjust one raster's "
			<< std::endl
			<< "elevations to match another's using a selected algorithm. The mask is "
			<< std::endl
			<< "optional and used to limit the placement of samples to where elevations are "
			<< std::endl
			<< "known to be good. The output is an adjustment raster with the same extent "
			<< std::endl << "as the input raster, with the new resolution. "
			<< std::endl << " -t  -- type            The type of adjustment. "
			<< std::endl << "                        nn  - natural neighbours."
			<< std::endl << "                        pl  - plane fit. "
			<< std::endl
			<< "                        avg - shift vertically by the average difference. "
			<< std::endl
			<< "                        idw - inverse distance weighting (use -e switch for exponent; default 1). "
			<< std::endl << "                        sk  - Simple Kriging."
			<< std::endl
			<< " -m  -- mask            an optional raster whose non-zero pixels dictate the locations of allowable samples."
			<< std::endl
			<< " -r  -- resolution      the pixel size of the output."
			<< std::endl << " -s  -- samples         the number of samples."
			<< std::endl
			<< " -b  -- base file       the raster to adjust the adjustment raster to."
			<< std::endl
			<< " -a  -- adjustment      file: the raster to adjust."
			<< std::endl << " -o  -- output file     the output file."
			<< std::endl
			<< " -p  -- print           print a table with the points, samples, deltas and residuals."
			<< std::endl << " -ie -- idw exponent    The IDW decay value."
			<< std::endl
			<< " -ip -- idw neighbours  Number of neighbours to consider. Leave out to consider all."
			<< std::endl;
}
...
int main(int argc, char **argv) {
	...
	try {
		...
		if (type == "nn") {
			inter =
					new interp::naturalneighbour::NaturalNeighbourInterpolator();
		} else if (type == "pl") {
			...
			interp::kriging::SimpleKrigingInterpolator *sk =
					new interp::kriging::SimpleKrigingInterpolator(argc, argv);
			inter = sk;
		} else {
			throw "Unknown interpolator type.";
		}

		adjust(basefile, adjfile, maskfile, outfile, *inter, numSamples,
				resolution, print);
