...
public:
	...
	Box(int c0, int r0, int c1, int r1) :
			m_c0(c0), m_r0(r0), m_c1(c1), m_r1(r1) {
	}
...
public:
	...
	Cell(int col, int row, float value) :
			m_col(col), m_row(row), m_value(value) {
		m_id = ++__cell_id;
...
public:
	...
	std::vector<Cell> computeEdges(Grid<unsigned int> &grd) {
		...
		g_debug("Cells: " << (mg_maxc - mg_minc) * (mg_maxr - mg_minr)
				<< "; " << edgeCells.size());
		return edgeCells;
	}
...
public:
	...
	Config(std::string &input, std::string &vdir, std::string &rdir,
			...
			mg_minBasinArea(minBasinArea),
			mg_maxSpillDist(maxSpillDist),
			m_t(1) {
	}
	...
	void loadSeeds(std::string &seedFile, bool header = false) {
		...
		g_debug("Seeds loaded from file: " << m_seeds.size());
	}
	...
	int fillBasins(std::string &filename, float elevation) {
		...
		for (Cell seed : seeds()) {
			...
			g_debug("Seed: " << seed.id() << ": " << seed.col() << ", "
							<< seed.row());

			if (!m_dem.has(seed.col(), seed.row())) {
				g_debug("WARNING: Found a seed out of bounds.");
				continue;
			}
	...
	bool findSpillPoints(std::string &rfile, double elevation) {
		...
		for (size_t i = 0; i < m_basinList.size(); ++i) {
			for (size_t j = i + 1; j < m_basinList.size(); ++j) {
				...
				for (size_t k = 0; k < cells0.size(); ++k) {
					for (size_t l = 0; l < cells1.size(); ++l) {
						double dist = cells0[k].distance(cells1[l],
								m_dem.resolutionX(), m_dem.resolutionY());
						minDist = dist < minDist ? dist : minDist;
						if (dist <= mg_maxSpillDist)
							m_spillPoints.push_back(
									SpillPoint(cells0[k], cells1[l],
											elevation));
					}
				}
			}
	...
	void findMinima() {
		...
		for (int r = 0; r < m_dem.rows(); ++r) {
			for (int c = 0; c < m_dem.cols(); ++c) {
				...
				for (int rr = g_max(0, r - 1);
						!skip && rr < g_min(r + 2, m_dem.rows()); ++rr) {
					for (int cc = g_max(0, c - 1);
							!skip && cc < g_min(c + 2, m_dem.cols()); ++cc) {
		...
		g_debug("Seeds found from minima: " << m_seeds.size());
	}
};
...
void flood(std::string &input, std::string &seeds, std::string &vdir,
	...
	Config config(input, vdir, rdir, spill, start, end, step, minBasinArea,
			maxSpillDist);
	config.init();
	...
	while (elevation <= end) {
		...
		if (basins > 1 && !spill.empty()
				&& config.findSpillPoints(rfile, elevation) > 0)
...
int main(int argc, char **argv) {
	...
	try {
		...
		geotools::flood::flood(input, seeds, vdir, rdir, spill, start, end,
