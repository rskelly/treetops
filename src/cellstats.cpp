...
CellStatsFilter::CellStatsFilter() : m_chain(nullptr) {
}

bool CellStatsFilter::keepImpl(const LASPoint *) const {
    return true;
}
...
bool CellStatsFilter::keep(const LASPoint *pt) const {
...
bool ClassFilter::keepImpl(const LASPoint *pt) const {
...
bool QuantileFilter::keepImpl(const LASPoint *pt) const {
...
CellStats::CellStats() : m_filter(nullptr) {
}
...
std::list<LASPoint*> CellStats::filtered(const std::list<LASPoint*> &values) {
...
void CellStats::compute(const std::list<LASPoint*>&, double*) {
}
...
void CellMean::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellCount::compute(const std::list<LASPoint*> &values, double *result) {
    result[0] = filtered(values).size();
}


void CellMedian::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellMin::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellMax::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellSampleVariance::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellPopulationVariance::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellSampleStdDev::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
...
void CellPopulationStdDev::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] =  -9999.0;
    } else {
...
void CellSkewness::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] =  -9999.0;
    } else {
...
void CellKurtosis::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] =  -9999.0;
    } else {
...
void CellCoV::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] =  -9999.0;
    } else {
...
CellQuantile::CellQuantile(unsigned char quantile, unsigned char quantiles) : CellStats(),
    m_quantile(quantile), m_quantiles(quantiles) {
}

void CellQuantile::compute(const std::list<LASPoint*> &values, double *result) {
    result[0] =  0;
}


double computePArea(double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3, double z3) {
...
double computeFArea(const Face &face) {
    ...
    return computePArea(p1.x(), p1.y(), p1.z(), p2.x(), p2.y(), p2.z(), p3.x(), p3.y(), p3.z());
}
...
double densityFactor(const std::list<LASPoint*> &values, CellDensity &cellDensity, double avgDensity) {
    if (values.size() == 0 || avgDensity <= 0.0 || cellDensity.area() <= 0.0)
        return 1.0;
    double density;
    cellDensity.compute(values, &density);
...
double polyArea(const std::list<Point_3> &hull, const Plane_3 &plane, const Point_3 &centroid) {
    ...
    do {
        ...
        area += computePArea(it0->x(), it0->y(), z0, it1->x(), it1->y(), z1, centroid.x(), centroid.y(), centroid.z());
        it0++;
        it1++;
        if (it1 == hull.end())
            it1 = hull.begin();
 ...
 */
void CellRugosity::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] =  -9999.0;
    } else {
        ...
        double df = m_avgDensity > 0.0 ? densityFactor(filt, m_density, m_avgDensity) : 1.0;
...
CellGapFraction::CellGapFraction(unsigned char type, double threshold) : CellStats(),
    m_type(type),
...
void CellGapFraction::compute(const std::list<LASPoint*> &values, double *result) {
    std::list<LASPoint*> filt = filtered(values);
    if (!filt.size()) {
        result[0] = -9999.0;
    } else {
