...
std::map<std::string, int> types = { { "Minimum", TYPE_MIN }, { "Maximum",
		...
		{ "Population Variance", TYPE_PVARIANCE }, { "Population Std. Dev.",
				...
				TYPE_QUANTILE }, { "Median", TYPE_MEDIAN } };
std::map<std::string, int> attributes = { { "Height", ATT_HEIGHT }, {
...
void LasGrid::lasgrid(std::string &dstFile, std::vector<std::string> &files,
		std::set<int> &classes, int crs, int attribute, int type, double radius,
		double resolution, Bounds &bounds, unsigned char angleLimit, bool fill,
		bool snap) {
	...
	g_debug(
			"Raster size: " << cols << ", " << rows << "; Cell radius: "
					<< radius);
	...
	for (unsigned int j = 0; j < indices.size(); ++j) {
		...
		while (reader.ReadNextPoint()) {
			...
			for (int cc = g_max(0, c - offset);
					cc < g_min(cols, c + offset + 1); ++cc) {
				for (int rr = g_max(0, r - offset);
						rr < g_min(rows, r + offset + 1); ++rr) {
		...
		for (size_t i = 0; i < (size_t) cols * rows; ++i) {
			if (counts[i] > 0) {
				...
				if (size % 2 == 0) {
					int idx = size / 2;
					grid1.set(i,
							(qGrid[i]->at(idx - 1) + qGrid[i]->at(idx)) / 2.0);
				} else {
	...
	if (type == TYPE_COUNT) {
		...
		Raster<double> rast(dstFile, 1, bounds, resolution, -resolution,
				-9999.0, crs);
