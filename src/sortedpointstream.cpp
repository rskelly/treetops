...
uint64_t _row(const std::shared_ptr<LASPoint> &pt, const Bounds &bounds,
		double blockSize) {
	if (blockSize < 0) {
...
SortedPointStream::SortedPointStream(const std::list<std::string> &files,
		const std::string &cacheFile, double blockSize, bool rebuild, bool snap,
		uint32_t threads) :
		m_files(files), m_cacheFile(cacheFile), m_blockSize(blockSize), m_rebuild(
				rebuild), m_inited(false), m_file(nullptr), m_snap(snap), m_threads(
				threads) {
}
	...
	while (m_running || m_flush.size()) {
		...
		if (m_cache[row].size() >= CACHE_LEN || !m_running) {
			...
			if (m_cache[row].size() >= CACHE_LEN
					|| (!m_running && m_cache[row].size() > 0)) {
				//g_debug(" -- " << m_cache[row].size() << " " << m_running);
				for (uint32_t i = 0; i < (m_cache[row].size() / CACHE_LEN) + 1;
						++i)
					m_flush.push(row);
			}
		...
		*((uint64_t *) buf0) = nextIdx;
		...
		if (prevIdx != curIdx) {
			std::fseek(m_file,
					prevIdx * bufLen + HEADER_LEN + 2 * sizeof(uint64_t),
					SEEK_SET);
...
void SortedPointStream::init() {
	...
	m_mfile.reset(
			Util::mapFile("pointstats.cache", m_rebuild ? s : 0, false).release());
	...
	std::unique_ptr<MappedFile> tfile(
			Util::mapFile("pointstats-idx.cache", m_rebuild ? s : 0, false).release());
