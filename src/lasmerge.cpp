...
void merge(std::vector<std::string> &files, std::string &outfile, double minx,
	...
	for (unsigned int i = 0; i < files.size(); ++i) {
		...
		if (h.GetMinX() > maxx || h.GetMinY() > maxy || h.GetMaxX() < minx
				|| h.GetMaxY() < miny)
			continue;

		if (h.GetMinX() < bounds[0])
			bounds[0] = h.GetMinX();
		if (h.GetMaxX() > bounds[1])
			bounds[1] = h.GetMaxX();
		if (h.GetMinY() < bounds[2])
			bounds[2] = h.GetMinY();
		if (h.GetMaxY() > bounds[3])
			bounds[3] = h.GetMaxY();
		if (h.GetMinZ() < bounds[4])
			bounds[4] = h.GetMinZ();
		if (h.GetMaxZ() > bounds[5])
			bounds[5] = h.GetMaxZ();

		indices.push_back(i);

		liblas::Header::RecordsByReturnArray rr =
				h.GetPointRecordsByReturnCount();
		...
		while (r.ReadNextPoint()) {
			...
			if (pt.GetX() > maxx || pt.GetX() < minx || pt.GetY() > maxy
					|| pt.GetY() < miny)
				continue;
