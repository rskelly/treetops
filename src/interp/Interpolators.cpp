...
void SimpleKrigingInterpolator::computeVariogram(
		std::list<InterpPoint> &samples, std::list<VariogramPoint> &variogram) {
	...
	for (auto it0 = samples.begin(); it0 != samples.end(); ++it0) {
		for (auto it1 = samples.begin(); it1 != samples.end(); ++it1) {
			...
			if (std::isnan(dist)) {
				std::cerr << it0->x << ", " << it1->x << ":" << it0->y << ", "
						<< it1->y << std::endl;
				std::cerr << g_sq(it0->x - it1->x) << ":"
						<< g_sq(it0->y - it1->y) << std::endl;
				throw "Bad distance.";
			}
...
void SimpleKrigingInterpolator::showVariogram(interp::kriging::KrigeArgs &kargs,
...
void SimpleKrigingInterpolator::interpolate(Raster<float> &out,
	...
	std::cerr << "Params: nugget: " << kargs.nugget << std::endl << "range: "
			<< kargs.range << std::endl << "sill: " << kargs.sill << std::endl;
	...
	for (int r = 0; r < out.rows(); ++r) {
		for (int c = 0; c < out.cols(); ++c) {
			...
			for (auto it = samples.begin(); it != samples.end(); ++it) {
				double dist = sqrt(
						g_sq(out.toX(c) - it->x) + g_sq(out.toY(r) - it->y));
				d(i, 0) = dist;
				b(i, 0) = kargs.model(dist, kargs.nugget, kargs.sill,
						kargs.range);
				++i;
			}
 ...
 */
void IDWInterpolator::interpolate(Raster<float> &out,
		std::list<InterpPoint> &samples) {
	using namespace interp::detail;
				...
				for (int r = 0; r < rows; ++r) {
					...
					for (int c = 0; c < cols; ++c) {
						...
						for (auto it = samples.begin(); it != samples.end();
								++it) {
							double d = pow(
									_sdist(*it, out.toX(c + sc),
											out.toY(r + sr)), m_exponent);
...
#pragma omp critical
		...
		const int num =
				m_neighbours <= 0 ?
						samples.size() :
						(samples.size() >= m_neighbours ?
								m_neighbours : samples.size());
...
#pragma omp critical
				...
				for (int r = 0; r < rows; ++r) {
					...
					for (int c = 0; c < cols; ++c) {
						...
						const double query[2] = { out.toX(c + sc), out.toY(
								r + sr) };
 ...
 * of the samples.
 */
void AvgInterpolator::interpolate(Raster<float> &out,
		std::list<InterpPoint> &samples) {
 ...
 */
void mcentroid(Eigen::Matrix<double, Eigen::Dynamic, 3> &mtx, int rows,
		int xcol, int ycol, double *x, double *y) {
	*x = 0, *y = 0;
	for (int r = 0; r < rows; ++r) {
		*x += mtx(r, xcol);
		*y += mtx(r, ycol);
...
void PlanarInterpolator::interpolate(Raster<float> &out,
		std::list<interp::InterpPoint> &samples) {
	using namespace detail;
	using namespace Eigen;
	Matrix<double, 3, 1> params(3, 1);
	...
	for (int r = 0; r < out.rows(); ++r) {
		for (int c = 0; c < out.cols(); ++c)
			out.set(c, r,
					(float) (params(0) + (out.toX(c) - cx) * params(1)
							+ (out.toY(r) - cy) * params(2)));
	}
}
 ...
 */
void NaturalNeighbourInterpolator::interpolate(Raster<float> &out,
		std::list<InterpPoint> &samples) {
