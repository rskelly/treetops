...
public:
	...
	static void bandNames(std::vector<std::string> &names, int numQuantiles) {
		std::string n[8] = { "count", "sum", "min", "max", "mean", "median",
				"variance", "stddev" };
		for (int i = 0; i < 8; ++i)
			names.push_back(n[i]);
	...
	double median() {
		...
		if (std::isnan(m_median) && (num = count()) > 0) {
			...
			if (num > 0) {
				sort();
				if (num % 2 == 0) {
					m_median = (m_values[num / 2] + m_values[num / 2 - 1])
							/ 2.0;
				} else {
...
void doVector(const std::string &shapefile, const std::string &outfile,
		const std::string &layername, const std::vector<std::string> &files,
		const std::vector<int> &classes, int numQuantiles, int outType,
		int scanAngle) {
	...
	while ((feat = layer->GetNextFeature()) != nullptr) {
		...
		geom::Geometry *geom =
				(geom::Geometry *) feat->GetGeometryRef()->exportToGEOS(gctx);
	...
	for (unsigned int i = 0; i < files.size(); ++i) {
		...
		std::vector<geom::Coordinate> coords = { geom::Coordinate(h.GetMinX(),
				h.GetMinY()), geom::Coordinate(h.GetMaxX(), h.GetMinY()),
				geom::Coordinate(h.GetMaxX(), h.GetMaxY()), geom::Coordinate(
						h.GetMinX(), h.GetMaxY()), geom::Coordinate(h.GetMinX(),
						h.GetMinY()) };
		...
		if (roi->intersects(bounds)) {
			...
			while (r.ReadNextPoint()) {
				...
				if (std::find(classes.begin(), classes.end(),
						pt.GetClassification().GetClass()) == classes.end())
					continue;
	...
	for (auto stat = stats.begin(); stat != stats.end(); ++stat) {
		...
		if (numQuantiles > 0) {
			...
			for (int qt = 0; qt < numQuantiles + 2; ++qt)
				feat->SetField(("qt" + std::to_string(qt)).c_str(),
						quantiles[qt]);
		}
		OGRPolygon *poly = (OGRPolygon *) OGRGeometryFactory::createFromGEOS(
				gctx, (GEOSGeom) stat->geom());
...
void doRaster(const std::string &raster, const std::string &outfile, int band,
	...
	if (0
			!= b->RasterIO(GF_Read, 0, 0, cols, rows, dg.grid(), cols, rows,
					GDT_Byte, 0, 0))
	...
	for (unsigned int i = 0; i < files.size(); ++i) {
		...
		while (r.ReadNextPoint()) {
			...
			if (std::find(classes.begin(), classes.end(),
					(cls = pt.GetClassification().GetClass())) == classes.end())
				continue;
			int sar = pt.GetScanAngleRank();
			if (sar < -scanAngle || sar > scanAngle)
				continue;
	...
	if (outType == CSV) {
		...
		GDALDataset *dst = drv->Create(outfile.c_str(), cols, rows,
				numRasterBands, GDT_Float32, nullptr);
		...
		for (auto it = stats.begin(); it != stats.end(); ++it) {
			for (unsigned int c = 0; c < classes.size(); ++c) {
				...
				for (int b = 0; b < numBands; ++b) {
					...
					if (0
							!= band->RasterIO(GF_Write, 0, 0, cols, rows,
									fg.grid(), cols, rows, GDT_Float32, 0, 0))
...
void usage() {
	std::cerr
			<< "Produces a zonal stats style output for points within polygons or a raster.\n"
