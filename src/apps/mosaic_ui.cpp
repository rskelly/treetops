...
void MosaicForm::setupUi(QWidget *form) {
	...
	connect(btnSelectFiles, SIGNAL(clicked()), this,
			SLOT(selectFilesClicked()));
	connect(btnRemoveSelected, SIGNAL(clicked()), this,
			SLOT(removeFilesClicked()));
	...
	connect(txtDistance, SIGNAL(textChanged(QString)), this,
			SLOT(distanceChanged(QString)));
	connect(txtTileSize, SIGNAL(textChanged(QString)), this,
			SLOT(tileSizeChanged(QString)));
...
void MosaicForm::updateFileButtons() {
	...
	btnOrderDown->setEnabled(
			selected && lstFiles->currentRow() < lstFiles->count() - 1);
}
