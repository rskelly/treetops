...
private:
	...
	void run() {
		...
		try {
			l.lasgrid(m_parent->m_destFile, m_parent->m_lasFiles,
					m_parent->m_classes, m_parent->m_hsrid,
					m_parent->m_attribute, m_parent->m_type, m_parent->m_radius,
					m_parent->m_resolution, m_bounds, m_parent->m_angleLimit,
					m_parent->m_fill, m_parent->m_snap);
