...
public:
	...
	void resize(size_t count) {
		...
		if(m_limit && size < m_limit) {
			...
			_trace("Resizing in-memory storage to " << size);
			m_vdata.resize(count);
			m_data = (T*) m_vdata.data();
		} else {
			// If we're over the limit, remap.
			_trace("Resizing file-backed storage to " << size);
			m_data = nullptr;
			remap(size);
			if(!m_vdata.empty()) {
				// If there's vdata, write it and clear the array.
				_trace("Copying in-memory contents to file-backed storage.");
