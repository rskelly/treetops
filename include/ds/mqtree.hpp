	...
	void load(const std::string& path) {
		if(path != m_path) {
			...
			if((handle = open(path.c_str(), O_RDWR, 0777)) > 0) {
				...
				_runerr("Failed to open file for load: " << path << " (" << strerror(errno) << ")");
			}
		}
	}
			...
			if(makedir(m_path)) {
				if((handle = open(m_path.c_str(), O_RDWR|O_CREAT|O_TRUNC, 0777)) > 0) {
					...
					if(!write(handle, m_buf.data, m_buf.size))
						_runerr("Failed to write cache.");
					close(handle);
				} else {
					_runerr("Failed to open file for flush: " << m_path << " (" << strerror(errno) << ")");
				}
			} else {
				_runerr("Failed to create dir for flush: " << m_path << " (" << strerror(errno) << ")");
			}
			m_cache.resize(0);
		}
	}
	...
	size_t search(const T& pt, double radius, TIter& piter) {
		...
		if(contains(pt, radius)) {
			if(m_split) {
				...
				for(const T& item : c) {
					d = _sq(item.x() - pt.x()) + _sq(item.y() - pt.y());
					if(d <= r2) {
						piter = item;
						++count;
					}
	...
	public:
		...
		double dist(const T& pt) const {
			if(node) {
				...
				return std::sqrt(_sq(item.x() - pt.x()) + _sq(item.y() - pt.y()));
			}
		}
...
public:
	...
	void init(double minx, double miny, double maxx, double maxy, int maxDepth = 100, int cacheSize = 1000) {
		...
		double side = _max(maxx - minx, maxy - miny);
