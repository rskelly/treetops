#ifndef __CROWNS_HPP__
#define __CROWNS_HPP__
...
namespace dijital {
    ...
    namespace crowns {
		...
		namespace util {
			...
			/**
			 * Forward declaration of Top.
			 */
			class Top;
		}

		/**
		 * Configuration for the crowns process.
		 */
        namespace config {
        	 ...
        	 */
        	class G_DLL_EXPORT TopThreshold {
        	public:
        		float threshold; 	///<! The minimum height for a pixel to be considered a top.
        		 ...
        		 */
        		TopThreshold(float threshold, int window) :
        	 ...
        	 */
        	class G_DLL_EXPORT CrownThreshold {
        	public:
        		float threshold;	///<! The minimum height for a pixel to be considered a member of a crown.
        		float fraction;		///<! The minimum height as a proportion of the top height for a pixel to be considered a member of a crown.
        		float radius;		///<! The maximum radius, in map units, of a crown.
        		 ...
        		 */
        		CrownThreshold(float threshold, float fraction, float radius) :
        	...
        	public:
        		 ...
        		 */
        		TopConfig(int col, int row, int window) : 
					col(col), row(row), window(window) {}
        	 ...
        	 */
        	class G_DLL_EXPORT CrownConfig {
        	public:
        		float x;	///<! The projected x coordinate of the crown.
        		float y;	///<! The projected y coordinate of the crown.
        		float z;	///<! The height of the crown.
        		 ...
        		 */
        		CrownConfig(float x, float y, float z) :
        			x(x), y(y), z(z) {}

        		/**
        		 * Construct a default CrownConfig with default values (0, 0, 0).
        		 */
        		CrownConfig() : 
					CrownConfig(0, 0, std::numeric_limits<float>::lowest()) {}
        	};

			/**
			 * Forward declaration of the main crowns program configuration.
			 */
        	class CrownsAppConfig;

        	/**
        	 * Enum of mutable fields in the CrownsAppConfig object.
        	 */
			enum G_DLL_EXPORT CrownsAppConfigField : uint64_t {
        		BoundsField = 1L << 0,
				BuildIndex = 1L << 1,
				TableCacheSize = 1L << 2,
				RowCacheSize = 1L << 3,
			...
			/**
			 * A container for a GEOS crown geometry and its associated context.
			 */
			class G_DLL_EXPORT CrownGeom {
			public:
				GEOSGeometry* crown;
				GEOSContextHandle_t gctx;

				/**
				 * Construct a crown geometry from the GEOS geometry and its context.
				 * \param crown A pointer to the GEOS geometry.
				 * \param gctx The GEOS context.
				 */
				CrownGeom(GEOSGeometry* crown, GEOSContextHandle_t gctx) :
					crown(crown), gctx(gctx) {}

				CrownGeom() :
					CrownGeom(nullptr, nullptr) {}
			};
			...
			private:
				 ...
				 * 
				 * \param[out] plen The length of the geometry data as written.
				 * \param[out] ppso The position of the geometry as written.
				 * \param crown A pointer to the crown geometry.
				 * \param geom The crown geometry object.
				 */
				bool writeCrown(size_t& plen, size_t& ppos, CrownGeom& geom);
			...
			public:
				...
				/**
				 * \brief Attempt to load the TopConfig and crown for the given ID.
				 *
				 * \param id The top ID.
				 * \param[out] top A reference to a writable TopConfig.
				 * \param[out] crown A reference to a CrownGeometry object.
				 * \param withCrown If true, loads the crown geometry. Otherwise not.
				 * \return True if the record was found.
				 */
				bool find(int id, Top& top, CrownGeom& crown, bool withCrown = true);

				/**
				 * \brief Read the next available top and crown geometry.
				 *
				 * \param[inout] top A TopConfig.
				 * \param[inout] crown A reference to a CrownGeometry object.
				 * \param loadCrown If false, the crown is not loaded.
				 * \return True if a record was found.
				 */
				bool next(Top& top, CrownGeom& crown, bool loadCrown = false);

				/**
				 * \brief Read the next available TopConfig and crown geometry.
				 *
				 * \param[inout] top A TopConfig.
				 ...
				 * \return True if a record was found.
				 */
				bool next(Top& top, std::vector<unsigned char>& crownwkb, bool loadCrown = false);


				/**
				 * \brief Read the next available TopConfig.
				 *
				 * \param[inout] top A TopConfig.
				 * \return True if a record was found.
				 */
				...
				/**
				 * Return the number of top/crown records.
				 * \return The number of top/crown records.
				 */
				int count();
				 ...
				 *
				 * If the TopConfig isn't already in the DB it will be inserted at the end.
				 *
				 * \param[inout] top A Treetop.
				 * \param[inout] crown A CrownGeom object.
				 * \return True on success.
				 */
				bool insert(const Top& top, CrownGeom& crown);
				 ...
				 * \return True on success.
				 */
				bool insert(const Top& top);

				/**
				 * \brief Update the Top and Crown.
				 *
				 * \param[inout] top A TopConfig.
				 * \param[inout] crown A CrownGeom object.
				 * \return True on success.
				 */
				bool update(const Top& top, CrownGeom& geom);

				/**
				 * \brief Update the Top.
				 *
				 * \param[inout] top A TopConfig.
				 * \return True on success.
				 */
				bool update(const Top& top);
        	...
        	/**
        	 * Gives an implementor the ability to receive updates from a CrownsAppConfig object.
        	 */
        	class G_DLL_EXPORT CrownsAppConfigListener {
        	public:

        		/**
        		 * Called when the CrownsAppConfig object generates an update.
        		 *
        		 * \param config The originating CrownsAppConfig object.
        		 * \param field An value containing ORed field IDs given by the CrownsAppConfigField enum.
        		 */
        		virtual void configUpdate(CrownsAppConfig& config, long field) = 0;

        		virtual ~CrownsAppConfigListener() {}
        	};

        	/**
        	 * Contains configuration information for performing tree top extraction.
        	 */
            class G_DLL_EXPORT CrownsAppConfig {
            private:
                dijital::util::Bounds<float> m_bounds;				///<! Defines the boundaries of work to be performed. Every step of the process will be confined, including smoothing and searching.
                ...
                float m_smoothSigma;							///<! The std. deviation used for generating the Gaussian kernel. 0 < n <= 1.

                std::string m_topsDatabase;						///<! The path to the crowns database.
                std::string m_topsDatabaseDriver;				///<! The river to use for creating the database.
                std::vector<TopThreshold> m_topsThresholds;		///<! For pixels equal or above each height (float) use the given
                												...
                												///<! the previous one.
                float m_topsMaxNulls;							///<! The max proportion of pixels in a given kernel that are allowed
                ...
                bool m_crownsUpdateHeights;						///<! If true, the heights of the crowns will be updated
                ...
                CrownsAppConfigListener* m_listener;			///<! Listens for updates from the config object.
                ...
                std::unique_ptr<dijital::grid::Band<float>> m_chm;
                std::unique_ptr<dijital::grid::Band<float>> m_smoothed;
                bool m_smoothExisted;
                std::unique_ptr<dijital::grid::Band<uint32_t>> m_topsWindow;
                std::unique_ptr<dijital::grid::Band<uint32_t>> m_topsID;
                std::unique_ptr<dijital::grid::Band<uint32_t>> m_topsParent;
                std::unique_ptr<dijital::grid::Band<uint32_t>> m_crowns;

            	std::unique_ptr<dijital::ds::mqtree<dijital::crowns::util::Top>> m_topsTree; 	///<! Used to maintain a mapped tops database if both crowns
            ...
            public:
				 ...
				 * \exception Throws an exception when configuraton fails.
				 */
				void loadFromJSON(const std::string& config_file);

            	dijital::grid::Band<float>& chm();
            	dijital::grid::Band<float>& smoothed();
            	bool smoothExisted() const;
				void setSmoothExisted(bool);
            	dijital::grid::Band<uint32_t>& crowns();
            	dijital::grid::Band<uint32_t>& topsWindow();
            	dijital::grid::Band<uint32_t>& topsID();
            	dijital::grid::Band<uint32_t>& topsParent();
            	dijital::ds::mqtree<dijital::crowns::util::Top>& topsTree();
                ...
                // Build a CrownsAppConfig with defaults.
                CrownsAppConfig();
                ...
                void setListener(CrownsAppConfigListener* listener);
                CrownsAppConfigListener* listener();
                ...
                void setTopsDatabase(const std::string& filename);
                const std::string& topsDatabase() const;

                /**
                 * \brief Set the tops database driver.
                 *
                 * \param driver The driver.
                 * \param forced True if the program has forced the driver to change due to file size or errors.
                 */
                void setTopsDatabaseDriver(const std::string& driver, bool forced = false);
                const std::string& topsDatabaseDriver() const;
                ...
                void setSmoothSigma(float sigma);
                float smoothSigma() const;
                ...
                void setTopsMaxNulls(float nulls);
                float topsMaxNulls() const;
        ...
        namespace util {
             ...
             */
            class G_DLL_EXPORT Top {
            public:
                size_t pos;			///<! The position in the qtree's memory.
                size_t id;			///<! The ID of this top.
                size_t parentID;	///<! The ID of this top's parent.
                float ox, oy, oz; 	///<! Original x, y, z value
                float sx, sy, sz; 	///<! Smoothed x, y, z value.
                float groundZ;		///<! The height of the ground extracted from a DTM.
			...
			public:
				 ...
				 */
				Node(int id, int c, int r, float z, int tc, int tr, float tz);
			...
			/**
			 * Save crowns stored in mapped memory into a database.
			 *
			 ...
			 */
			void getKernelMax(Band<float>& raster, int col, int row, int window,
			 ...
			 * \param window The size of the window.
			 */
			void zeroKernel(Band<int>& raster, int col, int row, int window);
			 ...
			 */
			void findCrownMax(dijital::grid::Band<float>& chm, dijital::grid::Band<uint32_t>& crowns,
			 ...
			 */
			bool verifyCrownPixel(int c, int r, float z, float nodata, float res,
			 ...
			 */
			float maxCrownRadius(CrownsAppConfig& config);
			 ...
			 * \param config The configuration object.
			 */
			void updateOriginalCHMHeights(CrownsAppConfig& config);

			/**
			 * Delineate the crowns using the given crowns.
			 *
			 * \param config The configuration object.
			 */
			void delineateCrowns(CrownsAppConfig& config);

			/**
			 * Create a vector database of the crowns.
			 *
			 * \param tmpFile The temporary database used to store both crowns and crowns.
			 * \param config The configuration object.
			 */
			void polygonizeCrowns(CrownsAppConfig& config);



			/**
			 * Locates crowns and updates their properties in
			 * the treetop-tracking rasters: the window grid and the ID grid.
			 *
			 * \param config The configuration object.
			 */
			void stage1(CrownsAppConfig& config);

			/**
			 * Locates the parent crowns and assigns the IDs.
			 *
			 * \param config The configuration object.
			 */
			void stage2(CrownsAppConfig& config);
			 ...
			 * \param config The configuration object.
			 */
			void stage3(CrownsAppConfig& config);

			/**
			 * Describes the possible DB conversion states.
			 */
			enum G_DLL_EXPORT TTDBState {
        ...
        /**
         * The main class of the Crowns application. Contains the high-level
         * API for performing the process.
         */
        class G_DLL_EXPORT Crowns {
        private:
        	dijital::crowns::config::CrownsAppConfig m_config;

        public:

			Crowns();

			dijital::crowns::config::CrownsAppConfig& config();

            /**
             * A convenience method for smoothing the input raster before using it to
             * generate crowns or crowns.
             */
            void smooth();
             ...
             * version of the raster. The table should have been generated using the crowns() method
